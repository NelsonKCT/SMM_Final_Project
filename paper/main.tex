...
sequence and fast retweet drop below 11\% for half of the countries and text
similarity swings from 3\% to nearly 100\% --- for China all three cover
only 3--7\%. In a merged graph every account is
...
0.952 to 0.686 --- a drop of at least 0.17 on every single country
...
with only 10\% of the labeled source accounts it averages 0.958 F1-macro ---
statistically indistinguishable from its full-data 0.952, with per-country
differences within one standard deviation --- and its margin over DFA is
stable at every fraction ($+$0.200 at 10\%, $+$0.186 at 50\%, $+$0.177 at
100\%). Because training draws a fixed 128-account stratified batch per
country and epoch regardless of the pool size, the number of gradient updates
is constant across fractions; what shrinks is the diversity of labeled
accounts. Stability under a $10\times$ label reduction indicates that the
gated coordination signal is learnable from a small set of labeled accounts:
\method{}'s advantage does not depend on label volume.

\begin{table}[t]
\centering
\caption{Zero-shot test F1-macro (six-country average) vs.\ fraction of
labeled source-country data.}
\label{tab:data-eff}
\begin{tabular}{lccc}
\toprule
Method & 10\% & 50\% & 100\% \\
...
\method{} & \textbf{0.958} & \textbf{0.955} & \textbf{0.952} \\
\bottomrule
\end{tabular}
\end{table}
...
IOHunter campaign countries (average 0.775 $\rightarrow$ 0.952) and retaining
its full-data performance with only 10\% of the labeled source accounts. The
