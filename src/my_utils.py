...
def get_edge_index_from_networkx(network):
    ...
    try:
        ...
        is_consecutive = (min(node_ints) == 0) and (max(node_ints) == len(nodes) - 1)
    except (ValueError, TypeError):
        is_consecutive = False

    if is_consecutive:
        import scipy.sparse as sp
        adj = nx.to_scipy_sparse_array(network, format='coo', weight=None)
        ...
        if len(edges) > 0:
            row = [node_to_idx[u] for u, v in edges]
            col = [node_to_idx[v] for u, v in edges]
